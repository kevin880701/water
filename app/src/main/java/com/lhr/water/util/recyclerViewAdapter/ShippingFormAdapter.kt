...
class ShippingFormAdapter(val listener: Listener): ListAdapter<FormData, ShippingFormAdapter.ViewHolder>(LOCK_DIFF_UTIL) {
    ...
    inner class ViewHolder(private val binding: ItemForm2Binding): RecyclerView.ViewHolder(binding.root){
        ...
        fun bind(formData: FormData){
            binding.tvName.text = formData.formName
        }
    }
}